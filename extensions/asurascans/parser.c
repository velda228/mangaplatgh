...
// Вспомогательная функция для поиска элементов по классу
...
// Функция для получения списка манги (специфичная для AsuraScans)
EMSCRIPTEN_KEEPALIVE
char* get_manga_list_asura(const char* html, const char* url) {
    ...
    // Используем селектор div.grid > a[href] как в Rust версии
...
// Функция для получения деталей манги (специфичная для AsuraScans)
EMSCRIPTEN_KEEPALIVE
char* get_manga_details_asura(const char* html, const char* url) {
    if (current_html) {
        free(current_html);
    }
    if (current_url) {
...
// Функция для получения списка глав (специфичная для AsuraScans)
EMSCRIPTEN_KEEPALIVE
char* get_chapter_list_asura(const char* html, const char* url) {
    if (current_html) {
        free(current_html);
    }
    if (current_url) {
...
// Функция для получения страниц главы (специфичная для AsuraScans)
EMSCRIPTEN_KEEPALIVE
